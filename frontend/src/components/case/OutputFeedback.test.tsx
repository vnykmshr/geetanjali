...
describe('OutputFeedback', () => {
  ...
  describe('confidence indicator colors', () => {
    it('should show green for high confidence (>=0.8)', () => {
      const { container } = render(
        <OutputFeedback {...defaultProps} output={{ ...mockOutput, confidence: 0.85 }} />
      )
    ...
    it('should show yellow for medium confidence (0.6-0.8)', () => {
      const { container } = render(
        <OutputFeedback {...defaultProps} output={{ ...mockOutput, confidence: 0.7 }} />
      )
    ...
    it('should show red for low confidence (<0.6)', () => {
      const { container } = render(
        <OutputFeedback {...defaultProps} output={{ ...mockOutput, confidence: 0.4 }} />
      )
  ...
  describe('feedback interactions', () => {
    it('should call onFeedback with "up" when thumbs up clicked', () => {
      ...
      const { container } = render(<OutputFeedback {...defaultProps} onFeedback={onFeedback} />)
  ...
  describe('feedback visual state', () => {
    it('should show thumbs up as active when feedback is "up"', () => {
      const { container } = render(<OutputFeedback {...defaultProps} feedback="up" />)
    ...
    it('should show thumbs down as active when feedback is "down"', () => {
      const { container } = render(<OutputFeedback {...defaultProps} feedback="down" />)
  ...
  describe('negative feedback form', () => {
    it('should show feedback form when expandedFeedback matches output', () => {
      render(<OutputFeedback {...defaultProps} expandedFeedback={mockOutput.id} />)

      expect(screen.getByText('What could be improved? (optional)')).toBeInTheDocument()
      expect(screen.getByPlaceholderText("Tell us what wasn't helpful...")).toBeInTheDocument()
    })

    it('should not show feedback form when expandedFeedback is null', () => {
      render(<OutputFeedback {...defaultProps} expandedFeedback={null} />)

      expect(screen.queryByText('What could be improved? (optional)')).not.toBeInTheDocument()
    })
    ...
    it('should call onFeedbackTextChange when typing', () => {
      ...
      const textarea = screen.getByPlaceholderText("Tell us what wasn't helpful...")
      fireEvent.change(textarea, { target: { value: 'Not helpful' } })

      expect(onFeedbackTextChange).toHaveBeenCalledWith(mockOutput.id, 'Not helpful')
    })
