...
import { mockMessages, mockOutputs, mockMessagesWithRetries, mockOutputsMultiple } from '../test/fixtures'
...
describe('groupMessagesIntoExchanges', () => {
  ...
  it('should take the latest assistant message when there are retries', () => {
    const exchanges = groupMessagesIntoExchanges(mockMessagesWithRetries, mockOutputsMultiple)
  ...
  it('should correctly associate outputs with exchanges', () => {
    ...
    expect(exchanges[0].output?.result_json.executive_summary).toBe('This is a test summary of the ethical guidance.')
  })
